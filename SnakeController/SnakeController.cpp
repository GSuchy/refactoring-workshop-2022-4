      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        ...
        while (length--) {
            Segment seg;
            istr >> seg.x >> seg.y;
            OurBraveSnake.m_segments.push_back(seg);
        }
    } else {
        throw ConfigurationError();
    }
...
{
    auto tail = OurBraveSnake.m_segments.back();
    ...
    OurBraveSnake.m_segments.pop_back();
}

void Controller::addHeadSegment(Segment const& newHead)
{
    OurBraveSnake.m_segments.push_front(newHead);
...
{
    if (OurBraveSnake.isSegmentAtPosition(newHead.x, newHead.y) or isPositionOutsideMap(newHead.x, newHead.y)) {
...
{
    if (OurBraveSnake.isSegmentAtPosition(x, y) || isPositionOutsideMap(x,y)) {
        m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        return;
    }
